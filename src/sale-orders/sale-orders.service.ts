...
export class SaleOrdersService {
  ...
  /**
   * Calcular costo de envío basado en código postal O coordenadas GPS
   * 
   * Método 1 (código postal): Lógica por zonas predefinidas
   * Método 2 (lat/lng): Cálculo por distancia real usando MapTiler API
   * 
   * Tarifas por distancia (basadas en Correo Argentino):
   * - 0-10 km: $1500 (entrega en 24-48hs)
   * - 10-30 km: $2500 (entrega en 2-3 días)
   * - 30-100 km: $4000 (entrega en 3-5 días)
   * - 100-300 km: $6000 (entrega en 5-7 días)
   * - +300 km: $8000 (entrega en 7-10 días)
   */
  async calculateShipping(dto: CalculateShippingDto) {
    ...
    // Si solo se proporciona código postal, usar método por zonas
    if (dto.postalCode) {
      return this.calculateShippingByPostalCode(dto.postalCode);
    }

    throw new BadRequestException('Debe proporcionar código postal o coordenadas (latitude, longitude)');
  }

  /**
   * Calcular envío por código postal (método simplificado por zonas)
   */
  private calculateShippingByPostalCode(postalCodeStr: string) {
    ...
    if (isNaN(postalCode) || postalCode < 1000 || postalCode > 9999) {
      throw new BadRequestException('Código postal inválido. Debe ser un número de 4 dígitos.');
    }
  ...
  /**
   * Calcular envío por distancia real usando fórmula de Haversine
   * (distancia en línea recta entre dos puntos GPS)
   */
  private async calculateShippingByDistance(clientLat: number, clientLng: number) {
    try {
      ...
      if (!localLatStr || !localLngStr) {
        throw new BadRequestException('Configuración de ubicación del local incompleta');
      }
      ...
      // Estimar duración aproximada (asumiendo 40 km/h promedio en ciudad, 60 km/h en ruta)
      ...
      return {
        ...
        deliveryTime,
        currency: 'ARS',
        message: 'Costo de envío calculado por distancia GPS',
      };
    } catch (error) {
      console.error('Error calculando envío por distancia:', error);
